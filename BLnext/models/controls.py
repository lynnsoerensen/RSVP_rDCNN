...
from resources.rcnn_sat.rcnn_sat.b_net import b_layer
from resources.rcnn_sat.rcnn_sat.bl_net import BLConvLayer

# Below are identical definition as in resources.rcnn_sat.rcnn_sat but with an added argument on output_activation.

def readout(x, classes, output_activation):
    '''Readout layer
    '''
...
def b_net(input_tensor, classes, output_activation='softmax'):
    '''Defines a B model
    '''
    ...
    output_tensor = readout(x, classes, output_activation)

    return tf.keras.Model(inputs=input_tensor, outputs=output_tensor)

def b_d_net(input_tensor, classes, output_activation='softmax'):
    '''Defines a B-D model
    '''
...
def bl_net(input_tensor, classes, n_timesteps=8, cumulative_readout=False, output_activation='softmax'):
...
def makeModel_control(model_name, trainingSet='ecoset', activation='linear', outputLayer=None):
    ...
    model = eval(model_name + '_net')(input_layer, classes=classes, output_activation=activation)
    model.load_weights(str(weight_dir /f"{model_name}_{trainingSet}.h5"))
